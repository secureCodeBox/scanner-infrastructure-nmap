    ...
    tempHostList = xmlInput.map(host => {
        ...
        if (host.ports && host.ports[0].port) {
            ...
            newHost.openPorts = openPorts.map(portItem => {
                ...
                const ostype = portItem.service[0].$.ostype;
                const confidence = portItem.service[0].$.conf;
                const extrainfo = portItem.service[0].$.extrainfo;
                ...
                if (ostype) portObject.ostype = ostype;
                if (confidence) portObject.conf = confidence;
...
class NmapScan extends EventEmitter {
    ...
    initializeChildProcess() {
        ...
        this.child.on('error', err => {
            console.log(`Error in Nmap Scan: "${err.code}":"${err.message}"`);
